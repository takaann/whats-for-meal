...
- 同じ食材を主菜と副菜の両方で使うのは1品以内に留めること（被りすぎないように工夫する）
...
- レシピの前に、ユーザーの気分や体調に対する共感コメントを1文だけ添えること（例：「今日は疲れてるんですね。栄養たっぷりのメニューでリフレッシュしましょう！」など）
- 栄養バランスを考えて提案すること（たんぱく質、食物繊維、ビタミンなどを意識すること）
...
`.trim();
  ...
  if (allergies && Array.isArray(allergies) && allergies.length > 0) {
    systemPrompt += `
- 指定されたアレルゲンを含む料理は絶対に提案しないでください。
- ただし、条件が厳しくてレシピが思いつかない場合は、条件を守りつつ可能な限り近い提案を行ってください。
- レシピが出せないからといって沈黙したり、エラーで返すことのないようにしてください。`;
  }
